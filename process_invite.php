...
if($submit_value == 'Accept'){
	...
	createActivity('User joined project', 'User: ' . $sql_user['username'] . ' has joined the project', $timenow, $proj_id);
	/*
...
*/
	...
	if($user_id == $proj['admin_id']){
		...
		$sql_user = mysql_fetch_assoc(mysql_query("SELECT username FROM user WHERE id = '$user_id' "));
		createActivity('User was kicked', 'User: ' . $sql_user['username'] . ' was kicked from the project', $timenow, $proj_id);
	...
	if(empty($email) || !preg_match("/([\w\-]+\@[\w\-]+\.[\w\-]+)/",$email)){
		...
		if($user_id === $proj['admin_id']){
			...
			if($result_add == TRUE){
				sendInvite($user['email'], $proj['name']);
			}else{
				echo "Something went very wong..";
			}
